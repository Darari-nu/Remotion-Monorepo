...
export interface SharpRevealTextProps {
    text: string;
    mode?: 'wipe-right' | 'wipe-up' | 'slice-diagonal' | 'box-expand' | 'shutter' | 'circle-expand'; // Base mode, but we randomize internals
    fontSize?: number;
    color?: string;
                ...
                return (
                    <div key={lineIndex} style={{
                        ...
                        {line.split('').map((char, charIndex) => {
                            ...
                            if (durationInFrames) {
                                // Allow negative fadeOutDuration to push fade start beyond end (effectively no fade)
                                // Standard: fadeStart = duration - 10 (Fades out last 10 frames)
                                const fadeStart = durationInFrames - fadeOutDuration;

                                // Only apply if current frame is past start of fade
                                if (frame >= fadeStart) {
                            ...
                            // Drift Calculation (Continuous Floating with Randomness)
                            // Use seed to randomize phase, speed, and amplitude per character
                            // Adjusted to middle ground (Medium speed, Medium amplitude)
                            const dPhaseX = random(seed + 'dpx') * 2 * Math.PI;
                            const dPhaseY = random(seed + 'dpy') * 2 * Math.PI;
                            const dSpeedX = 20 + random(seed + 'dsx') * 20; // 20 to 40 frames divisor (Medium)
                            const dSpeedY = 25 + random(seed + 'dsy') * 25; // 25 to 50 frames divisor (Medium)
                            const dAmpX = 4 + random(seed + 'dax') * 6; // 4 to 10px amplitude (Medium)
                            const dAmpY = 4 + random(seed + 'day') * 6; // 4 to 10px amplitude (Medium)

                            const driftX = Math.sin((frame / dSpeedX) + dPhaseX) * dAmpX;
                            const driftY = Math.cos((frame / dSpeedY) + dPhaseY) * dAmpY;
                            ...
                            const containerStyle: React.CSSProperties = {
                                ...
                                transform: `translate(${jX + driftX}px, ${jY + driftY}px) rotate(${jR}deg)`,
